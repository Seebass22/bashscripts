...
if [ "$site" == twitch ]; then
	...
	qutebrowser -R "https://www.twitch.tv/popout/$channel/chat?popout=" > /dev/null 2>&1 & disown
	...
	qutebrowser -R "https://www.youtube.com/live_chat?$id&is_popout=1" > /dev/null 2>&1 & disown
	setsid mpv -quiet "$url" > /dev/null 2>&1 &
	sleep 2;
	herbstclient focus r
else
